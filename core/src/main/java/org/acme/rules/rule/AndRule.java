...
@Slf4j
public final class AndRule extends AbstractCompositeRule {

    private final ParameterExtractor parameterExtractor;

    @Builder
    AndRule(@NonNull CompositeRuleDescriptor ruleDescriptor, List<Rule> rules, boolean metricsEnabled, ParameterExtractor parameterExtractor) {
    ...
    protected RuleTrace executeInternal(Map<String, Object> input, Map<String, Object> globalParams, EvaluationContext evaluationContext) {
        ...
        try {
            ...
            if (super.getRuleDescriptor().isEnabled()) {
                ...
                for (Rule rule : super.getRules()) {
                    ...
                    if (ruleTrace.getChildren() != null) {
                        compositeResult = ruleTrace.getChildren().stream().allMatch(RuleTrace::isResult); //And condition -> the best effort execution of all rules regardless of previous result
                    }
                }
            } else {
            ...
            log.info("Rule name: {} - Successfully executed Composite Rule Expression [AND] | Result: {}", super.getRuleName(), compositeResult);
            return evaluationContext.exitRule(compositeResult, rulesExecuted != size, end, executionStatus);
        } catch (Exception ex) {
            long end = System.nanoTime() - start;
            log.error("Rule name: {} - Composite Rule Expression [AND] execution failed. Stacktrace: ", super.getRuleName(), ex);
