...
@Slf4j
public final class OrElseRule extends AbstractCompositeRule {

    private final ParameterExtractor parameterExtractor;

    @Builder
    OrElseRule(@NonNull CompositeRuleDescriptor ruleDescriptor, List<Rule> rules, boolean metricsEnabled, ParameterExtractor parameterExtractor) {
    ...
    protected RuleTrace executeInternal(Map<String, Object> input, Map<String, Object> globalParams, EvaluationContext evaluationContext) {
        ...
        try {
            ...
            log.info("Rule name: {} - Successfully executed Composite Rule Expression [OR_ELSE] | Result: {}", super.getRuleName(), compositeResult);
            return evaluationContext.exitRule(compositeResult, rulesExecuted != size, end, executionStatus);
        } catch (Exception ex) {
            long end = System.nanoTime() - start;
            log.error("Rule name: {} - Composite Rule Expression [OR_ELSE] execution failed. Stacktrace: ", super.getRuleName(), ex);
